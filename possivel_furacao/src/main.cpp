...
#define DEVICE_ID             "possivel_furacao"
...
void loop() {
  ...
  // ─────────── Faixas para POSSÍVEL FURACÃO ───────────
  float temperature = randomFloat(26.0, 32.0);  
  float humidity    = randomFloat(80.0, 90.0);  
  int   pressure    = random(960, 980);         
  ...
  // ─────────── Publica no tópico hurricane/possivel_furacao/data ───────────
  String topic = String("hurricane/") + DEVICE_ID + "/data";
  mqtt.publish(topic.c_str(), payload);

  Serial.printf("Possivel Furacao → %s\n", payload);
